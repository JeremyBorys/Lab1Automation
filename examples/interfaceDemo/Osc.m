...
classdef Osc < handle
    ...
    methods
        ...
        % Example:
            disp('Function: getChannels not implemented yet.')
            retVal = -1;
        end
        ...
        % Example:
            ...
            freq = obj.getFrequencyCH2();
            x = linspace(-1/(freq), 1/(freq), 2500);
            amp = obj.getPk2PkCH2();
            
            if obj.readWave() == 1
                y = amp.*sin((2*pi*freq).*x) + obj.readOff()/2;
            elseif obj.readWave() == 2
                y = amp.*sawtooth((2*pi*freq).*x) + obj.readOff()/2;
            elseif obj.readWave() == 3
                y = amp.*square((2*pi*freq).*x) + obj.readOff()/2;
            else
                disp('Should never end up here')
            end
            plot(x, y)
        ...
        function retVal = readOff(obj)
            fid = fopen('tempOffset.txt');
            retVal = fscanf(fid, '%f');
            fclose(fid);
        end
        
        function retVal = readWave(obj)
        ...
        % Example:
            ...
            x = linspace(-1/(freq), 1/(freq), 2500);
            amp = obj.getPk2PkCH2();
            y = amp .* sin((2*pi*freq) .* x) + obj.readOff();
            
            if obj.readWave() == 1
                y = amp.*sin((2*pi*freq).*x) + obj.readOff();
            elseif obj.readWave() == 2
                y = amp.*sawtooth((2*pi*freq).*x) + obj.readOff();
            elseif obj.readWave() == 3
                y = amp.*square((2*pi*freq).*x) + obj.readOff();
            else
                disp('Should never end up here')
            end
